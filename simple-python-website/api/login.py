...
def do_login(db):
    ...
    if user_id > 0:
        ...
        response.set_cookie(
            ...
            max_age=Config.config.session_ttl,
        )
...
def register(db):
    ...
    if (
        ...
        if users.user_by_username(db, username) is not None:
            ...
            registration_success = users.create_user(
                db, Config.config, firstname, lastname, username, password
            )
    ...
    response.headers["Content-type"] = "application/json"
    return dict(registration_success=registration_success, error_msg=error_msg)
