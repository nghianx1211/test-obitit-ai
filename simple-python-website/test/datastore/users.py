...
class UsersTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.config = Config(
            password_secret="really-long-password-secret-for-generating-a-hash"
        )
    ...
    def test_password_hash_stays_same(self):
        assert self.correct_hash == users.calculate_password_hash(
            self.config, self.correct_password
        )
        assert self.correct_hash == users.calculate_password_hash(
            self.config, self.correct_password
        )
    ...
    def test_password_hashes_are_different(self):
        assert users.calculate_password_hash(
            self.config, self.correct_password
        ) != users.calculate_password_hash(self.config, self.wrong_password)
